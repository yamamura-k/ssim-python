...
import numpy as np
import torch
from libcpp import ssim_batch_recursive, ssim_batch_full # , ssim_batch_hybrid
...
with torch.no_grad():
    for size in range(10, SIZE, 10):
        ...
        for r in range(10, n_images + 1, 10):
            print("-"*10 + f" Recursive: {r:>3} / Size: {size:>3} " + "-"*10)
            for mode in range(7):
