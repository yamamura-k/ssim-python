...
cdef cnp.ndarray[cnp.float32_t, ndim=2] _ssim(
    img1, img2, window, int window_size, int channel, bint size_average = True
):
    cdef:
        float C1, C2
...
cpdef cnp.ndarray[cnp.float32_t, ndim=2] _ssim_batch_full(images, window, int window_size = 11):
    cdef:
...
cpdef cnp.ndarray[cnp.float32_t, ndim=2] _ssim_batch_hybrid(images, window, int window_size, int channel, int q_matrix_threshold=100):
    cdef:
        int n_candidates, i
...
cpdef cnp.ndarray[cnp.float32_t, ndim=2] _ssim_batch_recursive(
    images, int l1, int r1, int l2, int r2, int q_matrix_threshold, window, int window_size = 11
):
    cdef:
        int m1, m2, length1, length2, bs1, bs2, channel, width, height
        float C1, C2
        cnp.ndarray[cnp.float32_t, ndim=2] _similarity1, _similarity2, _similarity12, _similarity21, similarity
    length1 = r1 - l1
    length2 = r2 - l2
