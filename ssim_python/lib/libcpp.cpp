...
torch::Tensor _ssim_batch_recursive_new3_subroutine(
    torch::Tensor images, torch::Tensor mu, torch::Tensor sigma, int l1, int r1, int l2, int r2, int q_matrix_threshold, torch::Tensor window, int window_size
) {
    torch::Tensor similarity;
    int length1 = r1 - l1;
    int length2 = r2 - l2;
    if (std::max(length1, length2) > q_matrix_threshold) {
        ...
        torch::Tensor _similarity1 = _ssim_batch_recursive_new3_subroutine(
            images, mu, sigma, l1, m1, l2, m2, q_matrix_threshold, window, window_size
        );
        torch::Tensor _similarity2 = _ssim_batch_recursive_new3_subroutine(
            images, mu, sigma, m1, r1, m2, r2, q_matrix_threshold, window, window_size
        );
        torch::Tensor _similarity12 = _ssim_batch_recursive_new3_subroutine(
            images, mu, sigma, m1, r1, l2, m2, q_matrix_threshold, window, window_size
        );
        if (l1 == l2) {
            _similarity21 = _similarity12.permute({1, 0});
        }
            ...
            _similarity21 = _ssim_batch_recursive_new3_subroutine(
                images, mu, sigma, l1, m1, m2, r2, q_matrix_threshold, window, window_size
            );
        }
        similarity = torch::cat(
            {
...
torch::Tensor _ssim_batch_recursive_new3(
    ...
    torch::Tensor similarity = _ssim_batch_recursive_new3_subroutine(images, mu, sigma_sq, l1, r1, l2, r2, q_matrix_threshold, window, window_size);
    return similarity;
}
...
torch::Tensor _ssim_batch_recursive_new4(
    ...
    torch::Tensor similarity = _ssim_batch_recursive_new4_subroutine(images, mu, sigma_sq, l1, r1, l2, r2, q_matrix_threshold, window, window_size);
    return similarity;
}
...
torch::Tensor _ssim_batch_recursive_put2_subroutine(
    torch::Tensor similarity, torch::Tensor mu, torch::Tensor sigma, torch::Tensor images, int l1, int r1, int l2, int r2, int q_matrix_threshold, torch::Tensor window, int window_size
) {
    int length1 = r1 - l1;
    int length2 = r2 - l2;
    if (std::max(length1, length2) > q_matrix_threshold) {
        ...
        _ssim_batch_recursive_put2_subroutine(
            similarity, mu, sigma, images, l1, m1, l2, m2, q_matrix_threshold, window, window_size
        );
        _ssim_batch_recursive_put2_subroutine(
            similarity, mu, sigma, images, m1, r1, m2, r2, q_matrix_threshold, window, window_size
        );
        _ssim_batch_recursive_put2_subroutine(
            similarity, mu, sigma, images, m1, r1, l2, m2, q_matrix_threshold, window, window_size
        );
        _ssim_batch_recursive_put2_subroutine(
            similarity, mu, sigma, images, l1, m1, m2, r2, q_matrix_threshold, window, window_size
        );
    } else {
        torch::Tensor images1 = images.index({Idx::Slice(l1, r1, Idx::None)});
        torch::Tensor images2 = images.index({Idx::Slice(l2, r2, Idx::None)});
